import QtQuick 2.0
import Felgo 3.0
import QtMultimedia 5.0





Scene {




    anchors.fill: parent.gameWindowAnchorItem




    Rectangle{





        id:lionLevel
        anchors.fill: parent.gameWindowAnchorItem


        Image {
            id: backd
            source: "../../assets/551727800a494_thumb900.jpg"
            anchors.fill:parent
            width: 1080
            height: 2280

        }






        StyledButton{
            id:nextButton
            visible: false
            color: "#00e600"
            radius: 0
            borderWidth: 2
            width: 230
            height: 60
            y:70
            borderColor: "yellow"
            x:50
            Text {
                id:forwardText
                x:20
                y:10
                font.pixelSize: 23
                text: "اجابة صحيحة"
                color: "#fff"


                Image {
                    x:130
                    y:5
                    width: 50
                    height: 30
                    source: "../../assets/grunge-arrow-2-4.png"
                }
            }

            onClicked: {
                var Component = Qt.createComponent("fan.qml");
                var window = Component.createObject(gameWindow);
                lionScene.visible=false
                window.show
            }
        }


        StyledButton{
            id:reloadButton
            visible: false
            color: "#f00"
            radius: 0
            borderWidth: 2
            width: 230
            height: 60
            y:70
            borderColor: "yellow"
            x:50
            Text {
                id:reloadText
                x:30
                y:10
                font.pixelSize: 23
                text: "اجابة خاطئة"
                color: "#fff"


                Image {
                    x:130
                    y:1
                    width: 40
                    height: 40
                    source: "../../assets/154-512.png"
                }
            }

            onClicked: {
                var Component = Qt.createComponent("computer.qml");
                var window = Component.createObject(gameWindow);
                lionScene.visible=false
                window.show
            }
        }







        StyledButton{
            id:backButton
            width:150
            color: "#eee"
            radius: 0
            borderWidth: 0

            y:10
            x:10



            Image {
                    width: parent.width*.25
                    height: 30
                    y:0

                    source: "../../assets/back-icon.png"
                    Text {

                        text: "القائمة الاساسية"
                        width:parent.width*.75
                        color: "#000"
                        font.pixelSize: 17
                        x:105
                        y:1
                }
            }
            onClicked: {
                var Component = Qt.createComponent("../Main.qml");
                var window = Component.createObject(gameWindow);
                lionScene.visible=false
                window.show
            }


        }
            Column{
                id:target
                x:(lionLevel.width-target.width)/2
                y:(lionLevel.height-target.height)/4+30
                Image {
                    source: "../../assets/Chooses/computer-img.png"
                    width:150
                    height:150

                }



        }

            Row{
                width: lionLevel.width



               StyledButton{
                   id:wrong1
                   x:lionLevel.width/10
                   y:(lionLevel.height)/1.5-20
                   borderWidth: 4
                   radius: 10


                       Text {
                           text: "ثعلب"
                           anchors.centerIn: parent
                           font.pixelSize: 20
                   }

                   color: "#fff"
                   borderColor: "#4dff4d"
                   width:70
                   height: 50




                   onClicked: {
                       reloadButton.visible = true
                   }

               }

               StyledButton{
                   id:wrong2
                   x:lionLevel.width/2.5
                   y:(lionLevel.height)/1.5-20
                   borderWidth: 4
                   radius: 10


                   Text {
                       text: "مروحة"
                       anchors.centerIn: parent
                       font.pixelSize: 20
               }

               color: "#fff"
               borderColor: "#4dff4d"
               width:70
               height: 50

               onClicked: {
                   reloadButton.visible = true
                   visible:false
               }

           }




               StyledButton{
                   id:corrAnswer
                   x:lionLevel.width/1.47
                   y:(lionLevel.height)/1.5-20

                   borderWidth: 4
                   radius: 10


                       Text {
                           text: "حاسوب"
                           anchors.centerIn: parent
                           font.pixelSize: 17
                   }

                   color: "#fff"
                   borderColor: "#4dff4d"
                   width:70
                   height: 50


                   onClicked: {
                       lionSound.play()
                       nextButton.visible = true

                       //-------------------------//


                      }





              SoundEffect{
                   id:lionSound
                   source: "..s/1.mp3"
                   loops: SoundEffect.Infinite
                 }

               }





            }

    }
}
